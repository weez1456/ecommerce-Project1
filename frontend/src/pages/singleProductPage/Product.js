...
import { addCartItems } from '../../redux/productSection/productSlice';
...
function Product(props) {
  ...
  const handleWishList = async (id, productId) => {
    try {
       await axios.put(
        `${apiUrl}/api/products/${id}/${productId}/wishlist`
      );
              ...
              <ListGroup variant="flush">
                <ListGroup.Item className="mb-3 ">
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <div style={{cursor:"pointer"}} onClick={()=> handleWishList(userInfo?._id, product?._id)}>
                    {userInfo?.wishList?.includes(product?._id)? <FavoriteIcon
                        style={{
                          width: '50px',
                          height: '50px',
                          color:"red",
